...
import numpy as np
import analytical_both_ends_fixed
...
def main(
    ...
    # Compute û0(x) = u0(x) - Cx - D
    C = (u0[-1] - u0[0])/xs_tsr[-1].item()
    D = u0[0]
    u0_hat = u0 - C * (xs_tsr.numpy()) - D
    u0_hat_tsr = torch.tensor(u0_hat)


    # Create the neural network
    neural_net = None
    architecture_tokens = architecture.split('_')
    if architecture_tokens[0] == 'ResidualNet':
        neural_net = architectures.ResidualNet(
    ...
    elif architecture_tokens[0] == 'Asine':
        neural_net = architectures.Asine(
            number_of_inputs=int(architecture_tokens[1]),
            number_of_blocks=int(architecture_tokens[2]),
            block_width=int(architecture_tokens[3]),
            number_of_outputs=int(architecture_tokens[4])
        )
    elif architecture_tokens[0] == 'Siren':
        neural_net = architectures.Siren(
            number_of_inputs=int(architecture_tokens[1]),
            layer_widths=ast.literal_eval(architecture_tokens[2]),
            number_of_outputs=int(architecture_tokens[3])
        )
    elif architecture_tokens[0] == 'ResSineNet':
        neural_net = architectures.ResSineNet(
            number_of_inputs=int(architecture_tokens[1]),
            number_of_blocks=int(architecture_tokens[2]),
            block_width=int(architecture_tokens[3]),
            number_of_outputs=int(architecture_tokens[4])
        ...
        neural_net = architectures.Wang2020(
            number_of_inputs=int(architecture_tokens[1]),
            number_of_blocks=int(architecture_tokens[2]),
            block_width=int(architecture_tokens[3]),
            number_of_outputs=int(architecture_tokens[4])
    ...
    with open(os.path.join(outputDirectory, "epochLoss.csv"), 'w') as epoch_loss_file:
        ...
        for epoch in range(1, schedule.last_epoch() + 1):
            ...
            #u = neural_net(diff_eqn_residual_x_t_tsr)  # (N_res, 1)
            #du_dx__du_dt = torch.autograd.grad(u, diff_eqn_residual_x_t_tsr, grad_outputs=u.data.new(u.shape).fill_(1), create_graph=True)[0]  # (N_res, 2)
            du_dx__du_dt = first_derivative(neural_net, diff_eqn_residual_x_t_tsr)
            #du_dx = du_dx__du_dt[:, 0]  # (N_res)
            du_dt = du_dx__du_dt[:, 1]  # (N_res)
            #d2u_dx2__d2u_dxdt = torch.autograd.grad(du_dx, diff_eqn_residual_x_t_tsr, grad_outputs=du_dx.data.new(du_dx.shape).fill_(1), create_graph=True)[0]  # (N_res, 2)
            d2u_dx2__d2u_dxdt = second_derivative(neural_net, diff_eqn_residual_x_t_tsr, 0)
    ...
    if displayResults:
        u_analytical = analytical_both_ends_fixed.main(alpha, duration,
        ...
        ax[2].plot(xs, u0, label='u0')
        ax[2].plot(xs, u0_predicted, label='u0 predicted')
        ax[2].plot(xs, u0_analytical, label='u0 analytical')
        ax[2].legend()
